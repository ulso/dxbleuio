...
fn header_title(sensor: &HibouAir) -> String {
    match sensor.get_board_type() {
        HibouAirType::TempHumSensor => "Temp/Humidity Sensor".to_string(),
...
#[component]
fn SensorPanelTempHum(sensor: HibouAir) -> Element {
    rsx! {
...
pub fn SensorPanel(sensor: HibouAir) -> Element {
    match sensor.get_board_type() {
        ...
        HibouAirType::TempHumSensor => rsx! {
            SensorPanelTempHum { sensor: sensor.clone() }
        },
        _ => rsx! {
